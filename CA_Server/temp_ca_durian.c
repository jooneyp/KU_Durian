	...
	while ((code = fgetc(fp)) != EOF) 
	{
        ...
        switch (code) {
            case CERT_VER :
				...
				cert_data->Ver = data[0];
				break;
            case CERT_SERIAL_NUMBER :
				cert_data->len_Cert_SN = len;
				memcpy(cert_data->Cert_SN, data, len);
                break;
			case CERT_ISSUER :
				cert_data->len_issuer = len;
				memcpy(cert_data->issuer, data, len);
				break;
            case CERT_VALID_FROM :
				cert_data->len_issueDate = len;
				memcpy(cert_data->issueDate, data, len);
				break;
            case CERT_VALID_TO :
				cert_data->len_expirationDate = len;
				memcpy(cert_data->expirationDate, data, len);
				break;
			case CERT_USER_NAME :
				cert_data->len_userName = len;
				memcpy(cert_data->userName, data, len);
				break;
			case CERT_REGISTRATION_NUMBER :
				cert_data->len_registrationNum = len;
				memcpy(cert_data->registrationNum, data, len);
				break;
			case CERT_PHONE_NUMBER :
				cert_data->len_phoneNum = len;
				memcpy(cert_data->phoneNum, data, len);
				break;
            case CERT_USIM_ID :
				cert_data->len_USIMID = len;
				memcpy(cert_data->USIMID, data, len);
				break;
            case CERT_USER_ID :
				cert_data->len_userID = len;
                memcpy(cert_data->userID, data, len);
				break;
            case CERT_USED_ALGORITHM :
				cert_data->len_usedAlgorithm = len;
				memcpy(cert_data->usedAlgorithm, data, len);
				break;
            case CERT_CA_PUBKEY :
				cert_data->len_pubKey = len;
				cert_data->len_pubKey_x = data[1];
				memcpy(cert_data->pubKey_x, data + 2, data[1]);
				cert_data->len_pubKey_y = data[35];
				memcpy(cert_data->pubKey_y, data + 36, data[35]);
                break;
            case CERT_SIGNATURE :
				cert_data->len_signature = len;
                memcpy(cert_data->signature, data, len);
                break;
            default :
                return ERR_CERT_TAG;
        }
...
SINT generate_PUB_CSR(CERT_INFO * cert, USER_INFO * user, UCHAR * in, UCHAR * out, SINT out_len, SINT check_in, SINT check_out)
{
	...
	if (check_out == 1)
	{
		...
		if (fp2 == NULL)
			return ERR_OPEN_FILE;
		fwrite(out, strlen((SCHAR *)temp_out), 1, fp2);
		out_len = NULL;
	}
		//버퍼일 경우
	else
...
SINT generate_signed_PUB(CERT_INFO * cert, UCHAR * in, UCHAR * out, UCHAR * salt, UINT salt_len, UINT iteration, SINT check_in, SINT check_out, SINT hash_alg, const UCHAR *d, ULONG d_len)
{
	...
	if (check_out == 1)
	{
		...
		if (fp == NULL)
			return ERR_OPEN_FILE;
		fwrite(out, strlen((SCHAR *)temp_out), 1, fp);
	}
	else
		memcpy(out, temp_out, strlen((SCHAR *)temp_out));
...
SINT revoke_PUB(CERT_INFO * cert, SINT cert_SN)
{
	time_t ltime;
    struct tm *today;
	...
	// 서명값 검증
	//ECDSA_verify_signature(SINT hash_alg, const UCHAR *qx, ULONG qx_len, const UCHAR *qy, ULONG qy_len, UCHAR *msg, ULONG msg_len, UCHAR *r, ULONG r_len, UCHAR *s, ULONG s_len)
	
	if (strncmp((SCHAR *)cert->expirationDate, currentTime, 4) == -1)
		part_cert2tlv(cert, temp_data);
	else if (strncmp((SCHAR *)cert->expirationDate, currentTime, 4) == 1)
	{	
		// 서명값 검증
		part_cert2tlv(cert, temp_data);
		ECDSA_verify_signature(SINT hash_alg, cert->pubKey_x, cert->len_pubKey_x, cert->pubKey_y, cert->len_pubKey_y, temp_data, strlen((SCHAR *)temp_data), UCHAR *r, ULONG r_len, UCHAR *s, ULONG s_len)
...
SINT part_cert2tlv(CERT_INFO * cert, UCHAR * temp_out)
{
	SINT offset;
	offset = 0;
	temp_out[offset] = CERT_VER;
