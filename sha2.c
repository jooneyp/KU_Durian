...
#define SHA1_STATE_SIZE			sizeof(SHA1_INFO)
...
#define R(x, n)         ((unsigned long long)((x)>>(n)))
...
#define S_512_old(x, n)   \
	(((x)>>((unsigned long long)(n) & PUT64(63))) | ((x) << ((unsigned long long)(64-((n)&PUT64(63))))))

#define S_512(x,s)	(((x)>>s) | (x)<<(64-s))

#define R_512(x, n)         ((x)>>((unsigned long long)n))
...
static const unsigned long long K_512[80] = {
...
static int SHA256_compute(SHA256_INFO *sha256, unsigned char *data);
static int SHA512_compute(SHA512_INFO *sha512, unsigned char *data);
...
int SHA224_final(SHA224_INFO *sha224, unsigned char *out)
{
...
int SHA256_update(SHA256_INFO *sha256, const unsigned char *data, unsigned int length) 
{
	...
	while (length > 0) {
		...
		if (!sha256->l2 && length >= SHA256_BLOCK_SIZE) {
			...
			if (!(SHA256_compute(sha256, (unsigned char *)data))) 
				return 0;
...
int SHA256_final(SHA256_INFO *sha256, unsigned char *out) 
{
	int i;
	int off = 0;
	...
	sha256->buf[sha256->l2++] = (unsigned char)0x80;
	...
	sha256->buf[56] = (unsigned char)(sha256->l1>>56);
	sha256->buf[57] = (unsigned char)(sha256->l1>>48); 			
	sha256->buf[58] = (unsigned char)(sha256->l1>>40); 		
	sha256->buf[59] = (unsigned char)(sha256->l1>>32); 	
	sha256->buf[60] = (unsigned char)(sha256->l1>>24); 
	sha256->buf[61] = (unsigned char)(sha256->l1>>16); 	
	sha256->buf[62] = (unsigned char)(sha256->l1>>8); 
	sha256->buf[63] = (unsigned char)(sha256->l1); 

	SHA256_compute(sha256, sha256->buf);

	for(i = 0; i < 8; i++) {
		off = i << 2;
		(out+off)[3] = (unsigned char)(sha256->data[i]);
		(out+off)[2] = (unsigned char)(sha256->data[i]>>8);
		(out+off)[1] = (unsigned char)(sha256->data[i]>>16);
		(out+off)[0] = (unsigned char)(sha256->data[i]>>24);
	}
	return 1;
}
...
static int SHA256_compute(SHA256_INFO *sha256, unsigned char *data) {
	...
	int i;
	unsigned int data_temp[8], W[64];
	unsigned int temp, t1, temp2;
	int off = 0;
	...
	for(i = 0; i < 16; i++) {
		off = i<<2;
		W[i] = (((unsigned int)((data+off)[0]<<24)) | 
...
int SHA384_final(SHA384_INFO *sha384, unsigned char *out)
{
...
int SHA512_update(SHA512_INFO *sha512, const unsigned char *data, unsigned int length)
{
	...
	while (length > 0)
	{
		...
		{
			if (!(SHA512_compute(sha512, (unsigned char *)data)))
				return 0;
		...
		{
			n = MIN((SHA512_BLOCK_SIZE - sha512->l2), length);
			...
			if (sha512->l2 == SHA512_BLOCK_SIZE) {
				if (!(SHA512_compute(sha512, sha512->buf)))
					return 0;
...
int SHA512_final(SHA512_INFO *sha512, unsigned char *out)
{
	int i, off=0;
	unsigned char *buf = sha512->buf;
	unsigned int n = sha512->l2;

	if (n >= SHA512_BLOCK_SIZE)
		return 0;

	sha512->l1 += n << 3;
	buf[n++] = (unsigned char)0x80;
	...
	buf[120] = (unsigned char)(((sha512->l1)>>56));
	buf[121] = (unsigned char)(((sha512->l1)>>48));
	buf[122] = (unsigned char)(((sha512->l1)>>40));
	buf[123] = (unsigned char)(((sha512->l1)>>32));
	buf[124] = (unsigned char)(((sha512->l1)>>24));
	buf[125] = (unsigned char)(((sha512->l1)>>16));
	buf[126] = (unsigned char)(((sha512->l1)>>8));
	buf[127] = (unsigned char)(((sha512->l1)));

	SHA512_compute(sha512, buf);

	for (i = 0; i < 8; i++) {
		off = i << 3;
		(out+off)[0] = (unsigned char)(((sha512->data[i])>>56));
		(out+off)[1] = (unsigned char)(((sha512->data[i])>>48));
		(out+off)[2] = (unsigned char)(((sha512->data[i])>>40));
		(out+off)[3] = (unsigned char)(((sha512->data[i])>>32));
		(out+off)[4] = (unsigned char)(((sha512->data[i])>>24));
		(out+off)[5] = (unsigned char)(((sha512->data[i])>>16));
		(out+off)[6] = (unsigned char)(((sha512->data[i])>>8));
		(out+off)[7] = (unsigned char)(((sha512->data[i])));
	}
	return 1;
}
...
static int SHA512_compute(SHA512_INFO * sha512, unsigned char *data)
{
	unsigned long long a,b,c,d,e,f,g,h, temp0, temp1;
	...
	{
		off = i << 3;
		W[i] = (((unsigned long long)((data + off)[0]))<<56)|(((unsigned long long)((data + off)[1]))<<48) | 
			(((unsigned long long)((data + off)[2]))<<40)|(((unsigned long long)((data + off)[3]))<<32) | 
			(((unsigned long long)((data + off)[4]))<<24)|(((unsigned long long)((data + off)[5]))<<16) | 
			(((unsigned long long)((data + off)[6]))<<8)|(((unsigned long long)((data + off)[7])));
	}
